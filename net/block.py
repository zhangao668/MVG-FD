...
class ResidualWithLearnedAlpha(nn.Module):
    def __init__(self):
        super(ResidualWithLearnedAlpha, self).__init__()
        self.alpha = nn.Parameter(th.ones(1) * 0.5)  # 初始化alpha为0.5，可以根据需要修改

    def forward(self, x, HH_up):

        x= x+ self.alpha * HH_up
        return x
...
class from_rgbd(nn.Module):
    def __init__(self, outchannels, use_eql=True):
        super(from_rgbd, self).__init__()
        if use_eql:
            self.conv_1 = _equalized_conv2d(3, outchannels, (1, 1), bias=True)
        else:
            self.conv_1 = nn.Conv2d(3, outchannels, (1, 1),bias=True)
